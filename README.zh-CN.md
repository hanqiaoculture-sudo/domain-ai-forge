...
Domain AI Forge 用统一接口把 **模型候选方案、领域评测 harness、多 agent workflow、工具环境模拟、轨迹评分** 放进同一个优化闭环，让每一次 prompt、模型、检索、工具或 agent 改动都能被度量。
...
- 只看最终回答，不看 agent 轨迹、工具调用和业务状态变化
- demo 成功以后，很难变成可运营产品
...
Tool Environment 是领域工具和业务状态的确定性模拟器。它能让 harness 判断系统到底有没有查订单、开工单、升级 case、改变状态，而不是只判断回答听起来是否合理。
...
Domain AI Forge 关注可运行、可评测、可复盘的垂域 AI 工程闭环：

- **3 分钟跑通**：不需要 API key 就能看到完整闭环
- **Domain Pack 生态**：让贡献者可以贡献一个行业包，而不是只能改核心代码
- **Benchmark 透明**：持续沉淀垂域 case 和候选系统对比
...
- 已有 harness scoring
- 已有 trajectory scoring
- 已有 tool environment simulator
- 已有 cost / latency / repeated-run reporting
- 已有 candidate comparison
- 已有 multi-agent composition
- 已有 customer support 示例
- 已有 JSON domain pack 和 JSONL case loader
- 已有 GitHub CI 和贡献入口
